...
if ( ($hldata = $rsData->fetch()) ){
    ...
    while ($row = $res->fetch()) {
        if ($row['UF_IMYAFAYLA'] && file_exists($_SERVER['DOCUMENT_ROOT'] . DokumentatsiyaManager::DIR_FILES . $row['UF_IMYAFAYLA'])) {
            $HLinfo[] = $row;
        }
    }
}
...
foreach ($arProd['XML_IDS'] as $prod){
    if (!in_array($prod['UF_NOMENKLATURA'], $arProdXmlId)){
        $arProdXmlId[] = $prod['UF_NOMENKLATURA'];
        $arProd['XML_IDS_NEW'][] = $prod;
    }
}

$elementsDB = CIBlockElement::GetList(['SORT' => 'ASC'],
                               ['IBLOCK_ID' => $core->getIblockId($core::IBLOCK_CODE_CATALOG), '=XML_ID' => $arProdXmlId, 'ACTIVE' => 'Y'],
                               false,
                               false,
                               ['IBLOCK_ID', 'ID', 'NAME']);
...
global $arrFilterTechnicalDocumentation;
$arrFilterTechnicalDocumentation = ["ID" => $ids];
?>

<?
$APPLICATION->IncludeComponent("citfact:document.elements", "technical_documentation", [
    "COMPONENT_TEMPLATE" => "technical_documentation",
    ...
    "FILTER_NAME" => "arrFilterTechnicalDocumentation",	// Имя массива со значениями фильтра для фильтрации элементов
    "HL_BLOCK_DOCS" => $arProd['XML_IDS_NEW'],
//    "PROPERTY_CODE" => array("ARTNUMBER","TYPE"),
