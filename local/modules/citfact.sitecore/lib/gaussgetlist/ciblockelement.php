 ...
 * @subpackage iblock
 */
namespace Citfact\SiteCore\GaussGetList;
...
class CIBlockElement extends \CAllIBlockElement
{
	...
	function prepareSql($arSelectFields=array(), $arFilter=array(), $arGroupBy=false, $arOrder=array("SORT"=>"ASC"))
	{
		...
		if(strpos($arSqlOrder[0],'PRICE_SCALE') > 0) {
			if(strpos($arSqlOrder[0],'asc') > 0) $arSqlOrder[0] = substr($arSqlOrder[0],0,-5).'*'.$arSqlOrder['PROPERTY_E_COMMERCE_VAL'];
			else $arSqlOrder[0] = substr($arSqlOrder[0],0,-6).'*'.$arSqlOrder['PROPERTY_E_COMMERCE_VAL'];
		}
