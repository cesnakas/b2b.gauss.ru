...
use Bitrix\Main,
	...
	Citfact\Sitecore\UserDataManager;
...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
	...
	protected function processRequest()
	{
		...
		$this->requestData["ID"] = ($_REQUEST["ID"]);
...
//		$arFilter["USER_ID"] = $USER->GetID();
		$arFilter["LID"] = SITE_ID;
			...
			if ($_POST['filter'] === 'id' && $_POST['value_id']) {
				if ($this->options['USE_ACCOUNT_NUMBER'])
					$arFilter['ACCOUNT_NUMBER'] = $_POST['value_id'];
				else
					$arFilter["ID"] = $_POST['value_id'];

			} elseif ($_POST['filter'] === 'date' && $_POST['value_date']) {

				$curDate = new \Bitrix\Main\Type\DateTime($_POST['value_date']);
				$lastDate = new \Bitrix\Main\Type\DateTime($_POST['value_date']);
				$arFilter[">=DATE_INSERT"] = $curDate;
				$lastDate->add("1 day");
				$arFilter["<DATE_INSERT"] = $lastDate;
			}
	...
	private function getRealId($id)
	{
        ...
        if(!empty($this->arParams['XML_ID'])){
            $arContragent = $this->arParams['XML_ID'];
        } else {
	...
	protected function obtainDataOrders()
	{
		...
		if ($this->sortBy == 'STATUS')
		{
			...
			$getListParams['order'] = array('ID' => $this->sortOrder);
		}
		else
		{
		...
		if ($this->offline) {
			...
			$res = \Bitrix\Sale\Internals\OrderPropsValueTable::getList([
				'filter' => ['CODE' => 'OFFLINE', 'VALUE' => 'Y'],
				'select' => ['ORDER_ID'],
			]);
			while ($row = $res->fetch()) {

                $arOfflineOrderIds[] = $row['ORDER_ID'];
				$arIds[] = $row['ORDER_ID'];
			}


			$getListParams['filter']['=ID'] = $arIds;
		...
		foreach ($orderIdList as $orderId)
		{
			...
			$this->dbResult['ORDERS'][$orderId] = array(
                ...
                "IS_OFFLINE" => in_array($orderId, $arOfflineOrderIds),
			);
		}
	}
	...
	protected function formatResult()
	{
		...
		$arResult["INFO"]["IS_OFFLINE"]  = $this->offline ? true : false;
		...
		$arResult['NAV_RESULT'] = $this->dbQueryResult['ORDERS'];
