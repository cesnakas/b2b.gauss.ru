...
class CBitrixPersonalOrderDetailComponent extends CBitrixComponent
{
	...
	protected function checkAuthorized()
	{
		...
		$this->countOrder = Order\OrderManager::checkOrders($this->arParams["ID"]);
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		$arParams["PATH_TO_CANCEL"] .= $arParams['ID'].'/';
		...
		if (self::isNonemptyArray($basketItems))
		{
			foreach ($basketItems as &$item)
			{
				...
				if($pict)
				{
					...
					{
						$arFileTmp = \Citfact\SiteCore\Pictures\ResizeManager::ResizeImageGet(
							$arImage,
								...
								foreach ($property["VALUES"] as $valId => $arValue)
								{
									...
									)
									{
										if ($property["TYPE"] == "S")
										{
											...
											$tmpImg = \Citfact\SiteCore\Pictures\ResizeManager::ResizeImageGet($arTmpFile, 30, 30);
											if (is_array($tmpImg))
											{
			...
			while ($property = $propertyData->Fetch())
			{
				...
				elseif ($property['PROPERTY_TYPE'] == \Bitrix\Iblock\PropertyTable::TYPE_ELEMENT)
				{
					...
					while ($arEnum = $rsPropEnums->fetch())
					{
						...
						$productImg = \Citfact\SiteCore\Pictures\ResizeManager::ResizeImageGet($arEnum['PREVIEW_PICTURE'], 80, 80);

						$arEnum['PREVIEW_PICTURE']['SRC'] = $productImg;
	...
	{
		if ($this->arResult["CANCELED"]!="Y" && $this->arResult["STATUS_ID"]!="F" && $this->arResult["PAYED"]!="Y")
		{
	...
	protected function formatResultUser()
	{
		...
		if (!empty($arResult['USER']) && is_array($arResult['USER'])) {
			...
			//имя оформившего закза
			$arResult['USER_NAME'] = CUser::FormatName(CSite::GetNameFormat(false), $arResult['USER'], true, false);

			//принадлежность оформившего заказ к группам менеджеров/ассистентов
			$arGroupAvailable = $core->GetGroupByCode($core::USER_GROUP_MANAGER.'|'.$core::USER_GROUP_ASSISTANT);

			$arGroups = CUser::GetUserGroup($arResult['USER']['ID']);
			$result_intersect = array_intersect($arGroupAvailable, $arGroups);

			$arResult['USER']['IS_MANAGER_OR_ASSISTANT'] = !empty($result_intersect) ? true : false;
		}
	}
