...
BasketPoolQuantity.prototype.updateQuantity = function()
{
	...
	if (!window.basketJSParams) {
		return;
	}

	if (window.basketJSParams['USE_ENHANCED_ECOMMERCE'] === 'Y')
	{
...
function updateBasketTable(basketItemId, res)
{
	...
	if (basketItemId !== null && !!res.BASKET_DATA)
	{
		...
		if (createNewItem)
		{
			...
			newRow.setAttribute('data-item-brand', arItem[window.basketJSParams['BRAND_PROPERTY'] + '_VALUE']);
		...
		if (createNewItem)
		{
			...
			for (i = 0; i < arColumns.length; i++)
			{
				switch (arColumns[i])
				{
					...
					case 'NAME':
						...
						{
							imageURL = window.basketJSParams.TEMPLATE_FOLDER + '/images/no_photo.png';
						}
					...
					case 'PRICE':
						...
						{
							oCellPrice.innerHTML += '<div class="type_price">' + window.basketJSParams['SALE_TYPE'] + '</div>';
			...
			if (bShowDeleteColumn || bShowDelayColumn)
			{
				...
				if (bShowDeleteColumn)
					oCellControl.innerHTML = '<a href="' + window.basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) + '">' + window.basketJSParams['SALE_DELETE'] + '</a><br />';

				if (bShowDelayColumn)
					oCellControl.innerHTML += '<a href="' + window.basketJSParams['DELAY_URL'].replace('#ID#', arItem['ID']) + '">' + window.basketJSParams['SALE_DELAY'] + '</a>';
			}
