...
{
    $arXMLID[] = $row['PROPS_ALL']["PRODUCT.XML_ID"]['VALUE'];
}
...
foreach ($this->basketItems as $row)
{
    ...
    $rowData = array(
		...
		'PRODUCT_XML_ID' => $row['PROPS_ALL']["PRODUCT.XML_ID"]['VALUE'],
...
//            unset($rowData['PROPS'][$k]);
    ...
    if ($rowData['AVAILABLE_QUANTITY'] >= 1000) {
        $rowData['OSTATOK_AVAILABLE'] = 'В наличии';
...
$basket = \Bitrix\Sale\Basket::loadItemsForFUser( \Bitrix\Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite())->getBasket();
...
foreach ($basket->getBasketItems() as $basketItem)
{
    ...
    if (in_array($basketItem->getProductId(), $this->arParams['ADD_ITEMS']) && $basketItem->isDelay() === false) {
        $totalSum += $basketItem->getFinalPrice();
    }
}

$result['TOTAL_RENDER_DATA']['PRICE'] = $totalSum;
$result['TOTAL_RENDER_DATA']['PRICE_FORMATED'] = CurrencyFormat($totalSum, 'RUB');
