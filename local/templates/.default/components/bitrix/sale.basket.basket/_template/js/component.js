;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		ids: {
			...
			totalDiscount: 'basket-total-price-discount',
			...
			itemListTitles: 'basket-item-list-titles',
        ...
        hideFilter: function()
        {
            ...
            BX.addClass(header, 'basket__inner--mobile');
        },
		...
		fillTotalBlocks: function()
		{
			...
			if (totalNodes && totalNodes.length)
			{
				...
				if (totalTemplate)
				{
					if (this.result.TOTAL_RENDER_DATA.NOT_AVAILABLE_SUM.indexOf('-') === 0) {
						this.result.TOTAL_RENDER_DATA.NOT_AVAILABLE_SUM = '0 ₽';
					}
					if (this.result.COUPON === 'TWO_PERCENT' && this.result.DISCOUNT_PRICE_ALL !== 0) {
						this.result.TOTAL_RENDER_DATA.CHECKBOX = 'checked';
						this.result.TOTAL_RENDER_DATA.COUPON = this.result.COUPON;
						
						if (!this.result.TOTAL_RENDER_DATA.PRICE_NEW) {
							this.result.TOTAL_RENDER_DATA.PRICE_FORMATED_NEW = this.result.TOTAL_RENDER_DATA.PRICE_FORMATED;
						}
					}
		...
		bindTotalEvents: function(node)
		{
			...
			if (this.result.TOTAL_RENDER_DATA.CHECKBOX) {
				BX.bind(this.getEntity(node, 'coupon-input-checkbox'), 'click', BX.proxy(this.removeCouponAction, this));
				let totalPrice = document.querySelector('#total-price');
				totalPrice.style.fontSize = '15px';
				totalPrice.style.textDecoration = 'line-through';
			} else {
				BX.bind(this.getEntity(node, 'coupon-input-checkbox'), 'click', BX.proxy(this.addCouponAction, this));
		...
		checkTotalAnimation: function(totalData)
		{
			if (this.result && this.result.TOTAL_RENDER_DATA && parseFloat(this.result.TOTAL_RENDER_DATA.PRICE) > parseFloat(totalData.PRICE))
			{
				...
				totalData.PRICE_WITHOUT_DISCOUNT_NEW = totalData.PRICE_WITHOUT_DISCOUNT;
				totalData.PRICE = this.result.TOTAL_RENDER_DATA.PRICE;

				totalData.PRICE_FORMATED_NEW = totalData.PRICE_FORMATED + '*';
				totalData.PRICE_WITHOUT_DISCOUNT_FORMATED_NEW = totalData.PRICE_WITHOUT_DISCOUNT_FORMATED;
				totalData.PRICE_FORMATED = this.result.TOTAL_RENDER_DATA.PRICE_FORMATED;

				this.addPriceAnimationData(this.ids.total, totalData.PRICE, totalData.PRICE_WITHOUT_DISCOUNT_NEW, totalData.CURRENCY);
			}
			return totalData;
		},
		...
		initializeBasketItems: function()
		{
			...
			this.getCacheNode(this.ids.itemListTable).innerHTML = '';
		...
		createBasketItem: function(itemId)
		{
			...
			if (basketItemTemplate)
			{
                ...
                this.animateUpakovkaByQuantity(this.items[itemId], this.items[itemId].QUANTITY);
			}
		},
		...
		editBasketItems: function(itemIds)
		{
			...
			for (i in itemIds)
			{
				...
				if (BX.type.isDomNode(BX(this.ids.item + item.ID)))
				{
					...
					this.applyQuantityAnimation(item);
				}
				else
				{
        ...
        applyQuantityAnimation: function(item)
        {
            var itemId = item.id;
            ...
            this.animateUpakovkaByQuantity(item, item.QUANTITY);
        },
		...
		setQuantity: function(itemData, quantity)
		{
			...
			if (quantityField)
			{
				...
				if (parseFloat(itemData.QUANTITY) !== parseFloat(quantity))
				{
					...
					this.animateUpakovkaByQuantity(itemData, quantity);
					this.actionPool.changeQuantity(itemData.ID, quantity, currentQuantity);
				}
			}
		},

		animateUpakovkaByQuantity: function(itemData, quantity)
		{
			var wrapNode, cntNode;

			if(window.innerWidth < 768) {
				wrapNode = $('#basket-item-'+ itemData.ID).find('.basket-item__text--m')[0];
				cntNode = $('#basket-item-'+ itemData.ID).find('.basket-item__text--m span')[0];
			} else {
				wrapNode = BX('basket-item-upakovka-wrap-' + itemData.ID);
				cntNode = BX('basket-item-upakovka-cnt-' + itemData.ID);
			}
			...
			var cntUpakovka = parseInt(itemData.KOLICHESTVO_V_UPAKOVKE);

			if (cntUpakovka) {
				let countBoxMod = quantity % cntUpakovka;
				let countBox = cntUpakovka - countBoxMod;
				if (countBoxMod !== 0) {
					wrapNode.classList.remove('hidden');
					cntNode.innerHTML = countBox;
				} else {
					wrapNode.classList.add('hidden');
					cntNode.innerHTML = countBox;
				}
				let btnAddBox = $(cntNode).closest('.basket-item__text').find('.btn-add-box');
				btnAddBox.attr('data-count-box-calc', quantity + countBox);
			}
