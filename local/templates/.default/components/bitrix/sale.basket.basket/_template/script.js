...
function saveCart() {
		Am.modals.showDialog('/local/include/modals/templateorder.php');
    return false;
}

function addBoxCount() {
	let $this = $(this);
	let countBoxCalc = $this.attr('data-count-box-calc');
	let input = $this.closest('.basket-item').find('.basket-item__count input.b-count__input');
	input.val(countBoxCalc);
	BX.fireEvent(input[0], 'change');
}

function successSaveCart(data) {
	if (data.status === 'success') {
		Am.modals.showDialog('/local/include/modals/templateorder.php?success=Y');
	} else {
		Am.modals.showDialog('/local/include/modals/templateorder.php?error=Y');
	}
}
...
function focusHundler() {
	var oldValueCountInput = '';

	this.focusinHundler = function() {
		oldValueCountInput = $(this).val();
		$(this).val('');
	};

	this.focusoutHundler = function() {
		if($(this).val() === '') {
			$(this).val(oldValueCountInput);
		}
	};
}

var focusHundlerObjext = new focusHundler();

document.addEventListener('App.Ready', function () {
	$('#clear-basket').on('click', clearCart);
	$('#save-basket').on('click', saveCart);
	$(document).on('click', '.btn-add-box', addBoxCount);

	$(document).on('focusout', '.basket-item__count input.b-count__input', focusHundlerObjext.focusoutHundler);
	$(document).on('focusin', '.basket-item__count input.b-count__input', focusHundlerObjext.focusinHundler);
});