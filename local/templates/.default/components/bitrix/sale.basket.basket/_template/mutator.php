...
foreach ($this->basketItems as $row)
{
    ...
    if($row['QUANTITY'] <= $row['AVAILABLE_QUANTITY']){
        $availableSum += $row['SUM_FULL_PRICE'];
    } else {
        $availableSum += $row['PRICE'] * $row['AVAILABLE_QUANTITY'];
    }
	...
	elseif (!empty($row['DETAIL_PICTURE_SRC']))
	{
        ...
        $rowData['IMAGE_URL'] =  '/local/client/img/no-photo.jpg';
    }
    ...
    /**
     * в корзину прокидываем свойства, чтобы поиск (фильтр) работал
     */
    $core = Core::getInstance();
    $arArticle = \CIBlockElement::GetProperty($core->getIblockId($core::IBLOCK_CODE_CATALOG), $rowData['PRODUCT_ID'], $by='', $order='', $filter = ['CODE' => 'CML2_ARTICLE'])->Fetch();
    $arUpakovka = \CIBlockElement::GetProperty($core->getIblockId($core::IBLOCK_CODE_CATALOG), $rowData['PRODUCT_ID'], $by='', $order='', $filter = ['CODE' => 'KOLICHESTVO_V_UPAKOVKE'])->Fetch();
    $product_properties = [
        [
            'NAME' => $arArticle['NAME'],
            'CODE' => $arArticle['CODE'],
            'VALUE' => $arArticle['VALUE'],
            'SORT' => $arArticle['SORT'],
        ],
        [
            'NAME' => $arUpakovka['NAME'],
            'CODE' => $arUpakovka['CODE'],
            'VALUE' => $arUpakovka['VALUE'],
            'SORT' => $arUpakovka['SORT'],
        ],
    ];
    $rowData['PROPS'] = array_merge($rowData['PROPS'], $product_properties);


    $rowData['CML2_ARTICLE'] = '';
    foreach ($rowData['PROPS'] as $k => $PROP) {
        if ($PROP['CODE'] == 'CML2_ARTICLE') {
            $rowData['CML2_ARTICLE'] = $PROP['VALUE'];
            /**
             * нельзя удалять, иначе поиск не работает,
             * если надо удалить, тогда в
             * local/templates/.default/components/bitrix/sale.basket.basket/_template/js/filter.js
             * в searchItemMatch дописать поиск по CML2_ARTICLE
             */
//            unset($rowData['PROPS'][$k]);
        }

        if ($PROP['CODE'] == 'KOLICHESTVO_V_UPAKOVKE') {
            $rowData['KOLICHESTVO_V_UPAKOVKE'] = $PROP['VALUE'];

            if (strpos($rowData['MEASURE_TEXT'], 'уп') === false) {
                $rowData['POKAZAT_KOLICHESTVO_V_UPAKOVKE'] = true;
            }
        }
	}
    ...
    $rowData['RESERV_BALANCE'] = $arResult['RESERV_BALANCE'][$rowData['PRODUCT_XML_ID']];
    if ($rowData['AVAILABLE_QUANTITY'] >= 1000) {
        $rowData['OSTATOK_AVAILABLE'] = 'Много';
        $rowData['OSTATOK_CLASS'] = 'green';

    } elseif ($rowData['AVAILABLE_QUANTITY'] <= 0) {
        $rowData['OSTATOK_AVAILABLE'] = 'Нет в наличии';
        $rowData['OSTATOK_CLASS'] = 'red';
        $rowData['OUT_OF_STOCK'] = true;

    } else {
        $rowData['RESERVE_AVAILABLE'] = $rowData['AVAILABLE_QUANTITY'];
        $rowData['RESERVE_VREZERVE'] = $rowData['RESERV_BALANCE']['UF_VREZERVE'];
    }

    foreach ($arResult['RESERV_BALANCE'] as $xmlId =>$balance){
        if(array_key_exists($rowData['PRODUCT_XML_ID'], $arResult['RESERV_BALANCE'])){
            $rowData['DATAPRIKHODA']= date("d-m-Y", strtotime($arResult['RESERV_BALANCE'][$xmlId]['UF_DATAPRIKHODA']));
        }
    }
...
$totalData = array(
    ...
    'DISCOUNT_PRICE_ALL' => $result['DISCOUNT_PRICE_ALL'],
	'CURRENCY' => $result['CURRENCY'],
    'AVAILABLE_SUM' => CCurrencyLang::CurrencyFormat($availableSum, 'RUB'),
    'NOT_AVAILABLE_SUM' => CCurrencyLang::CurrencyFormat($result['allSum'] - $availableSum, 'RUB')
);
