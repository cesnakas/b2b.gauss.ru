...
//Получение информации о голосующем.
global $USER;
$rsUsers = CUser::GetList(($by = "id"), ($order = "asc"), ['ID' => $USER->GetID()]);
$userInfo = $rsUsers->Fetch();
if (UserDataManager::isRegularUser()) {
    $contragentCurrent = UserDataManager::getContrAgentInfo();
    if ($contragentCurrent) {
        $name = $contragentCurrent['UF_NAME'];
    }
} else {
    $name = $userInfo['NAME'] . ' ' . $userInfo['LAST_NAME'] . ', сотрудник компании Gauss';
}
if (empty($name)) {
    $name = $userInfo['NAME'] . ' ' . $userInfo['LAST_NAME'];
}

$arResult['CONTRAGENT_INFO'] = htmlspecialchars($name);