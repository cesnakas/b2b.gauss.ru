...
function OrdersCnt($item)
{
    $count = Order\OrderManager::getOrdersCount();

    $item['TITLE'] = $item['TEXT'];
    $item['TEXT'] = $item['TEXT'] . ' <span>' . $count . '</span>';
    return $item;
}
    ...
    if ($obCache->InitCache(36000, serialize($arFilter), "/iblock/menu")) {
        ...
        if ($arIBlock = $dbIBlock->GetNext()) {
            ...
            while ($arSections = $dbSections->GetNext()) {
                ...
                if ($pictureSrc)
                    $arResizePicture = \Citfact\SiteCore\Pictures\ResizeManager::ResizeImageGet(
                        $arSections["PICTURE"],
                        240,
                        700
                    );
...
foreach ($arResult as $key => $arItem) {
    if ($arItem['LINK'] === '/personal/orders/') {
        $arItem = OrdersCnt($arItem);
    }

    // лист ожидания
    if ($arItem['LINK'] === '/personal/list-wait/') {
        $count = getUnseenWaitingListItems();
        if ($count > 0) {
            $arItem['TITLE'] = $arItem['TEXT'];
            $arItem['TEXT'] = $arItem['TEXT'] . ' <span>' . $count . '</span>';
        }
    }
...
/**
 * Возвращает кол-во непросмотренных доступных элементов в листе ожидания
 *
 * @return int
 */
function getUnseenWaitingListItems() {
    $count = 0;
    $listWait = [];
    ...
    if ($hlblock) {
        ...
        $rsData = $hlClassName::getList([
            'filter' => ['UF_USER_ID' => $USER->GetID(), 'UF_VIEWED' => false]
        ]);
            
        while ($arData = $rsData->Fetch()) {
            $ids[] = $arData['UF_PRODUCT_ID']; // массив продуктов для запроса
            $listWait[$arData['UF_PRODUCT_ID']] = $arData['UF_COUNT'];
        } 
    }
    if (!empty($ids)) {
        $res = \CIBlockElement::GetList([], ['ID' => $ids, 'ACTIVE' => 'Y'], false, [], ['ID', 'QUANTITY']);
        while ($arItem = $res->Fetch()) {
            if ($arItem['QUANTITY'] >= $listWait[$arItem['ID']]) {
                $count++;
            }
        }
    }
