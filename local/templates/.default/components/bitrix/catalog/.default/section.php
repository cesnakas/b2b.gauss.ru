...
if (!empty($userPriceType['ID'])) {
    ...
    $priceId = empty($basePrice) ? '' : $basePriceId;
}
...
<?php
if (
    ($arCurSection["DEPTH_LEVEL"] <= 3 && $arCurSection['ELEMENT_CNT'] == 0)
) { ?>
    ...
    <? if($arResult["VARIABLES"]["SECTION_CODE"] == 'svetodiodnye-lampy' && $arResult["VARIABLES"]["SECTION_ID"]) {
        $needSetTitle = true;
        $arSubSectFilter = [
            'IBLOCK_ID'=>$arParams["IBLOCK_ID"],
        ...
        $db_list = CIBlockSection::GetList(['SORT' => 'ASC'], $arSubSectFilter, true);
    ...
    foreach ($sections as $section){
        ...
        ?><? $APPLICATION->IncludeComponent(
            ...
            array(
                ...
                "ADD_SECTIONS_CHAIN" => $section["ADD_SECTIONS_CHAIN"],
            ),
            $component,
            array("HIDE_ICONS" => "Y")
        );?><br/><?
    ...
    if($needSetTitle){
        ...
        if($arResult["IPROPERTY_VALUES"]["SECTION_META_TITLE"]) {
            ...
            $APPLICATION->AddChainItem($arResult["IPROPERTY_VALUES"]["SECTION_META_TITLE"], '');
        } else {
            $sect = \Bitrix\Iblock\SectionTable::getList([
                'filter' => ['IBLOCK_ID' => $arParams["IBLOCK_ID"], 'ID' => $arResult["VARIABLES"]["SECTION_ID"]],
                'select' =>  ['NAME']
            ])->fetch();
            $APPLICATION->SetPageProperty("title", $sect["NAME"]);
            $APPLICATION->SetTitle($sect["NAME"]);
            $APPLICATION->AddChainItem($sect['NAME'], '');
        }
    }

}
?>
<?if ($arCurSection["DEPTH_LEVEL"] >= 4 || $arCurSection['ELEMENT_CNT'] > 0):?>
    <div class="c">
        <div class="aside">
            <?// КОЛОНКА ?>
            <div class="aside__sidebar">
                <?$APPLICATION->IncludeComponent(
                    ...
                    array(
                        ...
                        "COUNT_ELEMENTS" => $arParams["SECTION_COUNT_ELEMENTS"],
                        "TOP_DEPTH" => $arParams["SECTION_TOP_DEPTH"],
                        "SECTION_URL" => '/products/'.$arResult["URL_TEMPLATES"]["section"],
                        "VIEW_MODE" => $arParams["SECTIONS_VIEW_MODE"],
                        "SHOW_PARENT_NAME" => $arParams["SECTIONS_SHOW_PARENT_NAME"],
                        "HIDE_SECTION_NAME" => (isset($arParams["SECTIONS_HIDE_SECTION_NAME"]) ? $arParams["SECTIONS_HIDE_SECTION_NAME"] : "N"),
                        "ADD_SECTIONS_CHAIN" => 'N',
